...
<html lang="en">
    <head>
        ...
        <title>VTQ | Quản trị hệ thống - Hỗ trợ</title>
                    ...
                    <div class="sb-sidenav-menu">
                        <div class="nav">
                            ...
                            <div class="collapse" id="collapseFour" aria-labelledby="headingFour" data-bs-parent="#sidenavAccordion">
                                <nav class="sb-sidenav-menu-nested nav accordion" id="sidenavAccordionPages">
                                    <a class="nav-link" href="{{route('admin.qlyhotro')}}"> Liên hệ</a> {{-- Cập nhật route --}}
                                    <a class="nav-link" href="{{route('admin.qlytuyendung')}}">Tuyển dụng</a> {{-- Cập nhật route --}}
                                </nav>
                            </div>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Quản trị yêu cầu liên hệ</h1> {{-- Cập nhật tiêu đề --}}
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Bảng điều khiển</a></li>
                            <li class="breadcrumb-item active">Yêu cầu liên hệ</li> {{-- Cập nhật breadcrumb --}}
                        </ol>

                        <div class="card mb-4">
                            <div class="card-header">
                                <i class="fas fa-table me-1"></i>
                                Danh sách yêu cầu liên hệ {{-- Cập nhật tiêu đề bảng --}}
                            </div>
                            <div class="card-body">
                                <table id="datatablesSimple" class="table table-bordered table-hover text-center align-middle">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Nội dung</th> {{-- Cập nhật cột --}}
                                            <th>Ngày gửi</th> {{-- Cập nhật cột --}}
                                            <th>Xoá</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {{-- Giả định biến được truyền vào là $hoTroRequests --}}
                                        @foreach ($hoTro as $index => $request) {{-- Cập nhật tên biến và lặp --}}
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ $request->name }}</td> {{-- Cập nhật tên cột --}}
                                                <td>{{ $request->phone }}</td> {{-- Cập nhật tên cột --}}
                                                <td>{{ $request->email }}</td> {{-- Cập nhật tên cột --}}
                                                <td>{{ $request->message }}</td> {{-- Cập nhật tên cột --}}
                                                <td>{{ $request->created_at->format('d-m-Y H:i') }}</td> {{-- Sử dụng created_at --}}
                                                <td>
                                                    <form action="{{ route('admin.hotro.delete', $request->id) }}" method="POST"> {{-- Cập nhật route và method --}}
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Bạn có chắc chắn muốn xóa yêu cầu liên hệ này không?');">Xoá</button> {{-- Thêm xác nhận xóa --}}
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
